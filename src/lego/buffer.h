 ...
 * \file: buffer.h
 * \brief:
 * Created Date: Monday, November 2nd 2020, 9:30:48 pm
 * Author: raphael hao
 * Email: raphaelhao@outlook.com
 * --------
 * Last Modified: Friday, November 6th 2020, 11:24:24 am
...
class BufferManager {
 public:
  ...
  //!
  BufferManager(
      std::shared_ptr<nvinfer1::ICudaEngine> engine, const int batchSize = 0,
      ...
      : mEngine(engine), mBatchSize(batchSize)
  {
    ...
    for (int i = 0; i < mEngine->getNbBindings(); i++) {
      ...
      std::cout << i << "-th Bindings" << std::endl
                << "dims 0: " << dims.d[0] << std::endl
                << "dims 1: " << dims.d[1] << std::endl
                << "dims 2: " << dims.d[2] << std::endl
                << "dims 3: " << dims.d[3] << std::endl;
 ...
 private:
  void* getBuffer(const bool isHost, const std::string& tensorName) const
  {
    ...
    if (index == -1)
      return nullptr;
    return (
        isHost ? mManagedBuffers[index]->hostBuffer.data()
               : mManagedBuffers[index]->deviceBuffer.data());
  }

  void memcpyBuffers(
      const bool copyInput, const bool deviceToHost, const bool async,
